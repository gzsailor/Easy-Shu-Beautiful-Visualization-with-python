...
plt.rc('axes',axisbelow=False)    #使网格线置于图表下层
...
y2=np.array(df["INFO-Processing"])

width=0.35
fig=plt.figure(figsize=(5,4.5),dpi=100,facecolor='w') 

plt.bar(x,y1,width=width,color='#CEDB28',label='TOTAL')  #调整y1轴位置，颜色，label为图例名称，与下方legend结合使用
plt.bar(x+width,y2,width=width,color='#00AEF7',label='INFO-Processing')       #调整y2轴位置，颜色，label为图例名称，与下方legend结合使用
plt.xticks(x+width/2,x_lable,size=12)      #设置x轴刻度，位置,大小
plt.yticks(size=12)                        #设置y轴刻度，位置,大小
plt.grid(axis="y",c='k',linewidth=0.1,alpha=1)     #设置y轴网格线的颜色与粗细
                     #将y轴网格线置于底层
...
plt.savefig('商业图表_商业周刊1.pdf',bbox_inches='tight', pad_inches=0.3)
plt.show()   